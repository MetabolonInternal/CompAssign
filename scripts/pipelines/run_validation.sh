...
if [[ ! -f "scripts/experiments/active_learning/validate_active_learning_complete.py" ]]; then
    echo "ERROR: Validation script not found: scripts/experiments/active_learning/validate_active_learning_complete.py"
    exit 1
fi
...
if [[ "$VERBOSE" == true ]]; then
    ...
    python scripts/experiments/active_learning/validate_active_learning_complete.py 2>&1 | tee validation_output.log
    VALIDATION_EXIT_CODE=${PIPESTATUS[0]}
else
    # Run quietly
    echo -n "Progress: "
    python scripts/experiments/active_learning/validate_active_learning_complete.py > validation_output.log 2>&1 &
    PID=$!
...
if [[ -f "scripts/pipelines/visualize_validation_results.py" ]]; then
    echo "Generating plots..."
    # Capture output and filter warnings
    VIZ_OUTPUT=$(python scripts/pipelines/visualize_validation_results.py 2>&1)
    VIZ_EXIT_CODE=$?
