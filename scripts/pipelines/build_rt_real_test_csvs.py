...
def run_attach_chem_classes(filtered_parq: Path, lib_id: int, chem_classes: Path) -> Path:
    ...
    # Call the attach script via a subprocess to avoid import path issues.
    import sys
    import subprocess

    cmd = [
        sys.executable,
        str(REPO_ROOT / "scripts" / "pipelines" / "attach_chem_classes_and_filter.py"),
        "--input",
        str(filtered_parq),
        "--lib-mapping",
        str(lib_map),
...
def run_make_rt_csv(chemclass_parq: Path, lib_id: int, out_csv: Path) -> None:
    ...
    cmd = [
        sys.executable,
        str(REPO_ROOT / "scripts" / "pipelines" / "make_rt_prod_csv_from_merged.py"),
        "--input",
        str(chemclass_parq),
        "--species-mapping",
        str(species_mapping),
