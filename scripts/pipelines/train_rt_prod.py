#!/usr/bin/env python3
"""
Production RT CSV loader (no model training).

This module used to train the legacy non-ridge hierarchical RT model. That model has been removed.

`load_production_csv()` remains as a lightweight utility for normalising production-style RT CSVs
into the indexed arrays/dataframes used across the current ridge RT pipelines.

To train/evaluate the current RT models, use:
...
def load_production_csv(data_csv: Path, include_es_group1: bool = False) -> LoadedData:
    ...
    Optional:
      - ES_* columns can be included via include_es_group1. When enabled, we zero-mask ES_* to
        the Group 1 runs ("1 human blood") to avoid leaking group-specific surrogates.
    """
...
def main() -> None:
    raise SystemExit(
        "The legacy `scripts/pipelines/train_rt_prod.py` training CLI has been removed.\n"
        "Use `./scripts/run_rt_prod.sh` (ridge models) instead.\n"
        "This module now only provides `load_production_csv()` for compatibility."
    )
