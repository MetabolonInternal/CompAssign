...
def setup_sweet_spot_model_with_data(
    n_species=3,
    n_compounds=60,
    n_peaks_per_species=150,
    ...
    print("  Target: challenging multi-candidate overlap with realistic noise")

    gen = MultiCandidateGenerator(seed=seed)
    ...
    model = PeakAssignment(
        mass_tolerance_ppm=500.0,  # Must match generator tolerance scale
        rt_window_k=2.0,
        random_seed=seed,
    )
...
def setup_realistic_model_with_data(
    n_species=5, n_compounds=100, n_peaks_per_species=150, noise_fraction=0.5, seed=42
):
    ...
    for i in range(0, n_compounds, compounds_per_pathway):
        ...
        compound_rt_true[i : i + compounds_per_pathway] = rt_center + np.random.normal(
            0, rt_spread, min(compounds_per_pathway, n_compounds - i)
        )
    ...
    for _ in range(n_isomers):
        ...
        compound_rt_true = np.insert(
            compound_rt_true, orig_idx + 1, compound_rt_true[orig_idx] + np.random.uniform(0.5, 2)
        )
        n_compounds += 1
    ...
    for species in range(n_species):
        ...
        present_compounds = present_compounds[
            np.random.random(len(present_compounds)) > dropout_rate
        ]
            ...
            peak_list.append(
                {
                    ...
                    "rt": rt,
                    "intensity": intensity,
                    "true_compound": compound_idx
                    % (n_compounds - n_isomers),  # Map back to original compounds
        ...
        for _ in range(int(n_peaks_per_species * noise_fraction)):
            noise_type = np.random.choice(["random", "contaminant", "matrix"], p=[0.3, 0.4, 0.3])
            ...
            if noise_type == "random":
                ...
                noise_mass = compound_mass[mimic_idx] + np.random.uniform(
                    -0.005, 0.005
                )  # Very close mass
    ...
    model = PeakAssignment(
        mass_tolerance_ppm=10.0,  # 10 ppm tolerance
    ...
    poor_rt_compounds = np.random.choice(
        n_compounds - n_isomers, size=int((n_compounds - n_isomers) * 0.3), replace=False
    )
...
def setup_model_with_data(
    ...
    seed: int = 42,
    realistic: bool = True,
    use_sweet_spot: bool = True,
):
    """Create and initialize a model with multi-candidate synthetic data.

    This function always routes to the multi-candidate (sweet spot) generator
    to ensure 5–8 candidates per peak and avoid forbidden features. Arguments
    are retained for API stability.
    """
    ...
    # Generate simpler synthetic data for the model
    ...
    # Setup model
    model = PeakAssignment(mass_tolerance_ppm=5.0, rt_window_k=3.0, random_seed=seed)
...
def experiment_1_acquisition_comparison():
    ...
    for method in acquisition_methods:
        ...
        if rounds:
            ...
            print(
                f"  Initial -> Final Precision: {initial_results.precision:.3f} -> {final.metrics_after['precision']:.3f}"
            )
            print(
                f"  Initial -> Final Recall: {initial_results.recall:.3f} -> {final.metrics_after['recall']:.3f}"
            )
...
def experiment_5_edge_cases():
    ...
    try:
        ...
        features = {
            10001: np.array([1, 0, 0]),
            10002: np.array([0, 1, 0]),
            10003: np.array([0, 0, 1]),
        }
    ...
    try:
        ...
        select_batch(
            probs_dict=probs_dict, batch_size=1, acquisition_fn="mi", prob_samples_dict=prob_samples
        )
...
def experiment_6_convergence_analysis():
    ...
    if rounds:
        ...
        precision_improvement = (
            rounds[-1].metrics_after["precision"] - rounds[0].metrics_before["precision"]
        )
        ...
        print(
            f"  Initial -> Final Precision: {rounds[0].metrics_before['precision']:.3f} -> {rounds[-1].metrics_after['precision']:.3f}"
        )
        print(
            f"  Initial -> Final Recall: {rounds[0].metrics_before['recall']:.3f} -> {rounds[-1].metrics_after['recall']:.3f}"
        )
        ...
        for rd in rounds_data:
            print(
                f"    {rd['round']:2d}  |   {rd['precision']:.3f}   | {rd['recall']:.3f}  | {rd['f1']:.3f}  |  {rd['entropy_delta']:6.2f}"
            )
    ...
    for exp_name, results in experiments.items():
        ...
        if exp_name == "acquisition_comparison":
            ...
            for r in results:
                ...
                print(
                    f"{method:13} | {r.metrics['precision_gain']:14.3f} | {r.metrics['recall_gain']:11.3f} | {r.metrics['entropy_reduction']:17.2f}"
                )
            ...
            for r in results:
                ...
                print(
                    f"{oracle:16} | {r.metrics['final_precision']:15.3f} | {r.metrics['final_recall']:12.3f} | {r.metrics['final_f1']:8.3f}"
                )
            ...
            for r in results:
                ...
                print(
                    f"Diversity={use_div}: Avg distance={r.metrics['average_batch_diversity']:.3f}, "
            ...
            for r in results:
                ...
                print(
                    f"  {t:5.2f}   |   {r.metrics['final_precision']:.3f}   | {r.metrics['final_recall']:.3f}  | {r.metrics['final_f1']:.3f}  | {r.metrics['assignment_rate']:.3f}"
                )
            ...
            for r in results:
                print(
                    f"Over {r.config['n_rounds']} rounds with batch_size={r.config['batch_size']}:"
                )
                print(
                    f"  Precision: {r.metrics['initial_precision']:.3f} -> {r.metrics['final_precision']:.3f} "
                    f"(+{r.metrics['precision_improvement']:.3f})"
                )
                print(
                    f"  Recall: {r.metrics['initial_recall']:.3f} -> {r.metrics['final_recall']:.3f} "
                    f"(+{r.metrics['recall_improvement']:.3f})"
                )
                print(f"  Total entropy reduction: {r.metrics['total_entropy_reduction']:.2f}")
                print(
                    f"  Efficiency: {r.metrics['total_entropy_reduction']/r.metrics['annotations_used']:.4f} entropy/annotation"
                )
