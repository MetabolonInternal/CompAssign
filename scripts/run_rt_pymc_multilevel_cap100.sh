...
Options:
  ...
  --models <csv>               Comma-separated (default: baseline,subgroup,pooled_intercepts,pooled_slopes,pooled,comp_hier,chem_hier,lasso)
  ...
  --anchor-expansions <csv>    Comma-separated: none,poly2 (default: none,poly2)
  ...
  --advi-steps <int>           ADVI steps (default: 10000)
  --advi-draws <int>           ADVI posterior draws (default: 100)
  --advi-log-every <int>       Print ADVI progress every N iters (default: 500)

  --chunk-size <int>           Eval CSV chunk size (default: 50000)
  --log-every-chunks <int>     Eval progress frequency (default: 20)
...
MODELS_CSV="baseline,subgroup,pooled_intercepts,pooled_slopes,pooled,comp_hier,chem_hier,lasso"
METHOD="advi"
MAP_MAXEVAL="50000"
MAX_TRAIN_ROWS="0"
MAX_GROUPS="0"
...
ANCHOR_EXPANSIONS_CSV="none,poly2"
INCLUDE_ES_ALL="1"
...
if [[ "${METHOD}" == "advi" ]]; then
  METHOD_ARGS+=(--advi-steps "${ADVI_STEPS}" --advi-log-every "${ADVI_LOG_EVERY}" --advi-draws "${ADVI_DRAWS}")
elif [[ "${METHOD}" == "map" ]]; then
  METHOD_ARGS+=(--map-maxeval "${MAP_MAXEVAL}")
fi
...
total_jobs=$(( ${#libs[@]} * ${#anchor_expansions[@]} * n_models_pymc + (${RUN_LASSO} * ${#libs[@]}) ))
job_i=0
