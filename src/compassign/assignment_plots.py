...
def plot_feature_distributions(logit_df: pd.DataFrame, plots_path: Path):
    ...
    feature_cols = [col for col in logit_df.columns 
                   if col not in ['peak_id', 'species', 'compound', 'label', 
                                 'raw_prob', 'calibrated_prob', 'pred_prob']]
    
    # Select top 3 most informative features based on variance ratio
    feature_importance = []
    ...
    # If we have the new features, use them; otherwise fall back to basic
    if len(top_features) >= 3:
        features = top_features
    else:
        # Fallback to basic features if available
        features = ['mass_err_ppm', 'rt_z', 'log_intensity']
        features = [f for f in features if f in logit_df.columns][:3]
...
def plot_feature_importance(trace: az.InferenceData, logit_df: pd.DataFrame, plots_path: Path):
    ...
    if 'theta_features' in trace.posterior:
        ...
        coef_labels = [feature_cols[i] if i < len(feature_cols) else f'Feature {i}' 
        ...
        if not means:
            # No coefficients found, skip this plot
            print("Warning: No feature coefficients found in trace, skipping feature importance plot")
            plt.close(fig)
            return
    ...
    if len(means) > 0:
        ...
        ax.text(0.5, 0.5, 'No coefficients to display', ha='center', va='center', transform=ax.transAxes)
    ax.set_yticks(y_pos)
    ...
    # Select top features for correlation (same as importance plot)
    if 'theta_features' in trace.posterior and len(feature_cols) > 3:
        ...
        # Fallback to basic features if new structure not available
        selected_features = ['mass_err_ppm', 'rt_z', 'log_intensity'] if all(
            f in feature_cols for f in ['mass_err_ppm', 'rt_z', 'log_intensity']
        ) else feature_cols[:3]
