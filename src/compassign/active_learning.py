    ...
    for peak_id, probs in probs_dict.items():
        if acquisition_fn == 'fp':
            ...
            if prob_samples_dict is None or peak_id not in prob_samples_dict:
                raise ValueError("MI acquisition requires prob_samples_dict with posterior samples.")
...
class ActiveLearner:
    ...
    def select_next_batch(self,
                         probs_dict: Dict[int, np.ndarray],
                         batch_size: int,
                         features_dict: Optional[Dict[int, np.ndarray]] = None) -> List[int]:
        """
        ...
        available_probs = {
            pid: probs for pid, probs in probs_dict.items()
            if pid not in self.selected_peaks
        }
