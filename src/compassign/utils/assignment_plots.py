...
def create_assignment_plots(
    ...
    feature_names_list = [
        c
        for c in logit_df_eval.columns
        if c
        not in [
            "peak_id",
            "species",
...
def plot_logistic_coefficients(trace: az.InferenceData, plots_path: Path, feature_names: list):
    """Plot posterior distributions of assignment model coefficients."""
    # Require new-model variables
    if "theta_features" not in trace.posterior:
        raise ValueError(
            "Expected 'theta_features' in posterior; other structures are not supported"
        )
    ...
    if "theta0" in trace.posterior:
        ...
        ax.axvline(
            np.mean(samples), color="green", linewidth=2, label=f"Mean: {np.mean(samples):.3f}"
        )
...
def plot_feature_distributions(logit_df: pd.DataFrame, plots_path: Path):
    ...
    # Get available feature columns (excluding metadata)
    feature_cols = [
        col
    ...
    # Select most informative features based on variance ratio
    feature_importance = []
    for feat in feature_cols:
        if feat in logit_df.columns:
            true_vals = logit_df[logit_df["label"] == 1][feat]
            false_vals = logit_df[logit_df["label"] == 0][feat]
            # Use difference in means normalized by pooled std as importance
            if len(true_vals) > 0 and len(false_vals) > 0:
                mean_diff = abs(true_vals.mean() - false_vals.mean())
                pooled_std = np.sqrt((true_vals.std() ** 2 + false_vals.std() ** 2) / 2)
                if pooled_std > 0:
                    importance = mean_diff / pooled_std
    ...
    # Use whatever number of features were identified (may be <3)
    ...
    for idx, (feature, label) in enumerate(zip(features, labels)):
        ...
        parts = ax.violinplot(
            data_to_plot, positions=positions, widths=0.7, showmeans=True, showmedians=True
        )
...
def plot_roc_pr_curves(logit_df: pd.DataFrame, plots_path: Path):
    ...
    ax.plot(recall, precision, color="green", linewidth=2, label=f"PR curve (AUC = {pr_auc:.3f})")

    # Add baseline (random classifier)
    baseline = np.sum(y_true) / len(y_true)
    ax.axhline(
        baseline, color="red", linestyle="--", label=f"Baseline (prevalence = {baseline:.3f})"
    )
...
def plot_probability_calibration(logit_df: pd.DataFrame, plots_path: Path, threshold: float = 0.5):
    ...
    for i in range(n_bins):
        ...
        if i == 0:
            mask = (logit_df["pred_prob"] >= bin_edges[i]) & (
                logit_df["pred_prob"] < bin_edges[i + 1]
            )
        elif i == n_bins - 1:
            mask = (logit_df["pred_prob"] >= bin_edges[i]) & (
                logit_df["pred_prob"] <= bin_edges[i + 1]
    ...
    ax.hist(prob_true, bins=20, alpha=0.6, label="True assignments", color="green", density=True)
    ax.hist(prob_false, bins=20, alpha=0.6, label="False candidates", color="red", density=True)
...
def plot_confusion_matrix(assignment_results: Dict[str, Any], plots_path: Path):
    ...
    cm = _val(assignment_results, "confusion_matrix", {}) or _val(
        assignment_results, "assignment_confusion", {}
    )
...
def plot_feature_importance(trace: az.InferenceData, logit_df: pd.DataFrame, plots_path: Path):
    ...
    # New calibrated model with vector of features
    feature_samples = trace.posterior["theta_features"].values
    ...
    # Get feature names from logit_df
    feature_cols = [
        col
        for col in logit_df.columns
        if col
    ...
    means = np.mean(feature_samples, axis=0)
    stds = np.std(feature_samples, axis=0)

    # Sort by absolute mean and take top 10
    importance_idx = np.argsort(np.abs(means))[::-1][:10]
    coef_labels = [
        feature_cols[i] if i < len(feature_cols) else f"Feature {i}" for i in importance_idx
    ]
    means = means[importance_idx]
    stds = stds[importance_idx]
    ...
    if len(means) > 0:
        ...
        ax.text(
            0.5, 0.5, "No coefficients to display", ha="center", va="center", transform=ax.transAxes
        )
    ax.set_yticks(y_pos)
    ...
    # Get feature columns dynamically (excluding metadata columns)
    feature_cols = [
        col
        for col in logit_df.columns
        if col
    ...
    # Select top features for correlation (same as importance plot); require new-model params
    if "theta_features" in trace.posterior and len(feature_cols) > 0:
        # Use top 6 features for readable correlation matrix
        feature_samples = trace.posterior["theta_features"].values
        if len(feature_samples.shape) == 3:
            n_chains, n_draws, n_features = feature_samples.shape
            feature_samples = feature_samples.reshape(n_chains * n_draws, n_features)
        means = np.mean(feature_samples, axis=0)
        importance_idx = np.argsort(np.abs(means))[::-1][:6]
        selected_features = [feature_cols[i] for i in importance_idx if i < len(feature_cols)]
    else:
        raise ValueError("Expected 'theta_features' in posterior for correlation analysis")

    # Add label for correlation analysis
    features_with_label = selected_features + ["label"]
    features_df = logit_df[features_with_label]
    corr_matrix = features_df.corr()

    # Create cleaner labels for display
    display_labels = []
    for feat in features_with_label:
        if feat == "label":
            display_labels.append("Label")
        elif feat == "mass_err_ppm":
            display_labels.append("Mass Error")
        elif feat == "rt_z":
            display_labels.append("RT Z-score")
        elif feat == "log_intensity":
            display_labels.append("Log Intensity")
        elif feat == "rt_err_normalized":
            display_labels.append("RT Err Norm")
        elif feat == "abs_rt_z":
    ...
    ax.set_title("Feature Correlations (Top Features)")
...
def create_performance_comparison_plots(
    ...
    if assignments_file.exists():
        ...
        merged["intensity_bin"] = pd.cut(
            merged["log_intensity"],
            bins,
    ...
    summary_lines = [
        ...
        f"• RMSE: {ppc_results['rmse']:.3f}",
        f"• MAE: {ppc_results['mae']:.3f}",
        f"• 95% Coverage: {ppc_results['coverage_95']*100:.1f}%",
        "",
        "Peak Assignment Performance:",
        f"• Precision: {_val(assignment_results, 'precision', 0):.3f}",
        f"• Recall: {_val(assignment_results, 'recall', 0):.3f}",
        f"• F1 Score: {_val(assignment_results, 'f1_score', 0):.3f}",
        "",
        "Data Statistics:",
        f"• Total RT observations: {len(obs_df)}",
        f"• Total peaks: {len(peak_df)}",
        f"• True peaks: {peak_df['true_compound'].notna().sum()}",
        f"• Decoy peaks: {peak_df['true_compound'].isna().sum()}",
    ]
    summary_text = "\n".join(summary_lines)
