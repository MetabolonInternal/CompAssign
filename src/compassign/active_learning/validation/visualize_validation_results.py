...
def plot_acquisition_comparison(data):
    ...
    acq_experiments = [
        exp for exp in data["experiments"] if exp["experiment_name"] == "acquisition_comparison"
    ]
    ...
    ax1 = axes[0, 0]
    ax1.bar(
        methods, entropy_reductions, color=["red" if x < 0 else "green" for x in entropy_reductions]
    )
    ...
    # Plot 2: FP Reduction
    ax2 = axes[0, 1]
    ax2.bar(methods, fp_reductions, color=["red" if x < 0 else "green" for x in fp_reductions])
...
def plot_oracle_robustness(data):
    ...
    oracle_experiments = [
        exp for exp in data["experiments"] if exp["experiment_name"] == "oracle_robustness"
    ]
...
def plot_threshold_calibration(data):
    ...
    threshold_experiments = [
        exp for exp in data["experiments"] if exp["experiment_name"] == "threshold_calibration"
    ]
...
def plot_convergence_analysis(data):
    ...
    convergence_exp = next(
        (exp for exp in data["experiments"] if exp["experiment_name"] == "convergence_analysis"),
        None,
    )
...
def plot_summary_dashboard(data):
    ...
    thresholds = [
        exp["config"]["threshold"]
        for exp in sorted(threshold_experiments, key=lambda x: x["config"]["threshold"])
    ]
    f1s = [
        exp["metrics"]["final_f1"]
        for exp in sorted(threshold_experiments, key=lambda x: x["config"]["threshold"])
    ]
...
def main():
    ...
    try:
        ...
        print("Please run: ./src/compassign/active_learning/validation/run_validation.sh first")
        return
