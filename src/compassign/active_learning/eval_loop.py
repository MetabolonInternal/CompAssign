...
from ..assignment.peak_assignment import PeakAssignment
from ..assignment.presence_prior import PresencePrior
from .core import select_batch
...
def calculate_expected_fp(probs_dict: Dict[int, np.ndarray], threshold: float) -> float:
    """
...
def simulate_annotation_round(
        ...
        if hasattr(assignment_model, "presence") and assignment_model.presence is not None:
            ...
            if compound_idx is not None:
                rt_observations.append(
                    {"species": species_idx, "compound": compound_idx, "rt": peak_row["rt"]}
                )

    if verbose:
        print(f"\nCollected {len(labeled_peaks)} annotations")
        n_null = sum(1 for label_value in labels_provided if label_value == 0)
    ...
    if verbose:
        ...
        fp_delta = expected_fp_after - expected_fp_before
        print(f"  Expected FP: {expected_fp_after:.3f} (Δ = {fp_delta:+.3f})")
        precision_delta = metrics_after["precision"] - metrics_before["precision"]
        print(f"  Precision: {metrics_after['precision']:.3f} (Δ = {precision_delta:+.3f})")
        recall_delta = metrics_after["recall"] - metrics_before["recall"]
        print(f"  Recall: {metrics_after['recall']:.3f} (Δ = {recall_delta:+.3f})")
...
def run_annotation_experiment(
    ...
    for round_num in range(1, n_rounds + 1):
        ...
        available_peaks = [
            pid for pid in assignment_model.train_pack["peak_ids"] if pid not in annotated_peaks
        ]
        ...
        if selection_method == "random":
            batch_peaks = np.random.choice(
                available_peaks, size=min(batch_size, len(available_peaks)), replace=False
            ).tolist()
        else:
    ...
    if verbose:
        ...
        if results:
            ...
            print(
                f"Precision improvement: {initial_precision:.3f} -> {final_precision:.3f} "
...
def compare_oracles(
    ...
    for oracle in oracles:
        ...
        fresh_model = PeakAssignment(
            mass_tolerance_ppm=assignment_model.mass_tolerance_ppm,
            rt_window_k=assignment_model.rt_window_k,
            rt_std_floor=assignment_model.rt_std_floor,
        ...
        if oracle_results:
            ...
            print(
                f"  Precision: {initial.metrics_before['precision']:.3f} -> "
                f"{final.metrics_after['precision']:.3f}"
            )
            print(
                f"  Expected FP: {initial.expected_fp_before:.3f} -> "
