...
from src.compassign.utils import SyntheticDataset, load_chemberta_pca20
...
def main() -> None:
    ...
    if args.cross_species:
        ...
        if len(train_df) == 0 or len(test_df) == 0:
            # If the generator produced too few runs per species (e.g. 1 run/species),
            # per-species run holdout can degenerate to an empty train split. Fall back
            # to a global run holdout split, and finally a row-level split as a last resort.
            all_runs = np.sort(run_df["run"].unique())
            if all_runs.size >= 2:
                n_hold = max(1, int(round(float(args.test_size) * int(all_runs.size))))
                n_hold = min(n_hold, int(all_runs.size) - 1)
                holdout_runs = np.sort(rng.choice(all_runs, size=n_hold, replace=False))
                train_df = rt_df[~rt_df["run"].isin(holdout_runs)].reset_index(drop=True)
                test_df = rt_df[rt_df["run"].isin(holdout_runs)].reset_index(drop=True)
            if len(train_df) == 0 or len(test_df) == 0:
                train_df, test_df = train_test_split(
                    rt_df, test_size=float(args.test_size), random_state=args.seed, shuffle=True
                )
