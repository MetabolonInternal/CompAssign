...
from .run_metadata import ensure_run_metadata
...
class HierarchicalRTModel:
    ...
    This class implements the production configuration used by the RT pipeline:
    - cluster → species hierarchy,
    - descriptor‑based compound baseline (``compound_features`` required),
    - class‑pooled run‑covariate coefficients γ (no global_gamma toggle),
    - non‑centred parameterisation with sum‑to‑zero constraints.

    For experimental configurations (class‑only baselines, global γ, ablations),
    use ``rt_hierarchical_experimental.HierarchicalRTModel`` instead.
    """
    ...
    def __init__(
        ...
        *,
        ...
        compound_features : array-like or DataFrame
            Compound descriptor matrix of shape (n_compounds × d). This is required
            in the production model; omit only when using the experimental model.
        ...
        # Descriptor matrix is required in the production model.
        if compound_features is None:
            raise ValueError(
                "Production HierarchicalRTModel requires 'compound_features'. "
                "For class-only baselines or ablations, use "
                "rt_hierarchical_experimental.HierarchicalRTModel instead."
            )
        ...
        self.compound_features: np.ndarray = z_arr
    ...
    def build_model(self, obs_df: pd.DataFrame) -> pm.Model:
        ...
        with pm.Model() as model:
            ...
            mu_log_sigma = (
                np.log(0.02) if self.sigma_y_loc is None else float(self.sigma_y_loc)
            )
            ...
            sigma_y_group = pm.Deterministic(
                "sigma_y_group", pm.math.exp(log_sigma_y_group)
            )
            ...
            sigma_y = pm.Deterministic(
                "sigma_y", pm.math.sqrt(pm.math.mean(pm.math.sqr(sigma_y_group)))
            )
            ...
            sigma_species = (
                pm.HalfNormal(
                    "sigma_species",
                    sigma=0.5 if self.sigma_species_scale is None else float(self.sigma_species_scale),
                )
                if self.use_species
                else None
            )
            ...
            if self.n_covariates > 0:
                ...
                # Per-feature scaling: ES_* covariates receive a slightly
                # tighter prior (scale factor < 1) relative to IS_* so that
                # their coefficients behave like strong but regularised
                # predictors. Fall back to a vector of ones when no ES_*
            ...
            if self.species_compound_intercept:
                ...
                pairs_struct = np.core.records.fromarrays(
                    [pairs[:, 0], pairs[:, 1]], names="s,c"
                )
                ...
                delta_sc_raw = pm.Normal(
                    "delta_sc_raw", 0.0, 1.0, shape=int(len(unique_pairs))
                )
    ...
    def predict_new(
        ...
        # Group-specific noise scales (per cluster); sigma_y is retained as a
        # scalar summary but not used directly for predictive variance.
