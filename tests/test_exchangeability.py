...
def _compute_probs_from_pack(
    train_pack, presence: PresencePrior, theta0, theta_null, theta_features
):
    """
...
def test_candidate_order_invariance():
    ...
    model = PeakAssignment(mass_tolerance_ppm=150.0, rt_window_k=10.0, random_seed=0)
    ...
    # Fixed weights (repeat pattern so it matches the current feature count)
    base_weights = np.array([0.8, -0.6, 0.2, -0.1], dtype=float)
    theta_features = np.resize(base_weights, len(model.feature_names)).astype(float)
    theta0 = 0.05
    theta_null = -0.2

    # Compute baseline probabilities
    base_rows = _compute_probs_from_pack(
        model.train_pack, model.presence, theta0, theta_null, theta_features
    )
    ...
    perm_rows = _compute_probs_from_pack(
        perm_pack, model.presence, theta0, theta_null, theta_features
    )
