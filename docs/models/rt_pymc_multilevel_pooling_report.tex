...
\texttt{window} is computed from the held-out residuals as a multiple of the residual scale (approximately ``$6\sigma$''):
\[
  w_{\mathrm{base}} = 6\cdot \mathrm{sd}(|\hat{y}-y|),
\]
...
\midrule
208 & Ridge (supercategory) & 0.009023 & 0.004673 & 0.943 & \textbf{0.023534} & \textbf{100.0\%} & \textbf{0.0} \\
208 & Ridge (partial pooling) & \textbf{0.007874} & \textbf{0.003821} & \textbf{0.953} & 0.026681 & \textbf{100.0\%} & 107.8 \\
208 & Lasso (supercategory) & 0.015081 & 0.007956 & -- & -- & 94.7\% & -- \\
\midrule
209 & Ridge (supercategory) & 0.008317 & 0.004677 & 0.920 & \textbf{0.021393} & \textbf{100.0\%} & \textbf{0.0} \\
209 & Ridge (partial pooling) & \textbf{0.007707} & \textbf{0.004285} & \textbf{0.956} & 0.027518 & \textbf{100.0\%} & 182.9 \\
...
\nolinkurl{./scripts/sally-dev evaluate} and compared:
\begin{itemize}
  ...
  (model type \texttt{COMPASSIGN\_PP\_RIDGE}).
\end{itemize}
...
\paragraph{What changed in Sally (summary).}
To support \texttt{COMPASSIGN\_PP\_RIDGE} end-to-end inside \texttt{sally-dev}, we made the following focused changes:
\begin{itemize}
  \item \textbf{New modelType.} Added \texttt{modelType=COMPASSIGN\_PP\_RIDGE} which loads CompAssign stage-1 coefficient
  summaries (\texttt{stage1\_coeff\_summaries\_posterior.npz} + \texttt{partial\_pool\_backoff\_summaries.npz}) and
  computes calibrated, per-task RT windows from the predictive variance.
  \item \textbf{RT window plumbing.} Generalized \texttt{RegressionPredictor} so RT windows can be either per-compound
  (1D) or per-(task,compound) (2D), since \texttt{COMPASSIGN\_PP\_RIDGE} produces row-specific uncertainty.
  \item \textbf{sally-dev flag pass-through.} Added \texttt{--compassign-rt-species} and
  \texttt{--compassign-rt-species-cluster} (ontology inputs), plus \texttt{--compassign-rt-covariates-csv} and
  \texttt{--compassign-rt-allow-missing-task-covariates} (feature parity via a production-style RT covariates CSV). Also
  added \texttt{--force-curate} to run the full curation pipeline for evaluation even when a sample set would normally be
  skipped.
  ...
  \texttt{COMPASSIGN\_PP\_RIDGE} loader so production-style RT covariates can be mounted into the Docker container and read
...
\nolinkurl{external_repos/sally/scripts/sally-dev evaluate} (baseline \texttt{ESLASSO} vs candidate
\texttt{COMPASSIGN\_PP\_RIDGE}) and writes outputs into fixed subdirectories under \nolinkurl{external_repos/sally/out/}.
...
\texttt{jw/new-experimental-model}.
...
baseline and \texttt{COMPASSIGN\_PP\_RIDGE} were run with the same peak-assignment method and evaluation mode.
...
6 & 12307 & COMPASSIGN\_PP\_RIDGE (partial pooling ridge) & \textbf{0.9473} & \textbf{0.8205} & \textbf{0.7846} \\
\midrule
8 & 12609 & ESLASSO (supercategory lasso) & \textbf{0.8553} & 0.6513 & 0.5867 \\
8 & 12609 & COMPASSIGN\_PP\_RIDGE (partial pooling ridge) & 0.8290 & \textbf{0.7464} & \textbf{0.6468} \\
\midrule
6 & 12725 & ESLASSO (supercategory lasso) & 0.6808 & \textbf{0.8610} & 0.6134 \\
6 & 12725 & COMPASSIGN\_PP\_RIDGE (partial pooling ridge) & \textbf{0.8774} & 0.8174 & \textbf{0.7336} \\
\bottomrule
\end{tabular}
\end{table}
...
Supercat & SSID & Model & Precision & Recall & TS \\
\midrule
8 & 20159 & ESLASSO (supercategory lasso) & \textbf{0.8865} & \textbf{0.8697} & \textbf{0.7825} \\
8 & 20159 & COMPASSIGN\_PP\_RIDGE (partial pooling ridge) & 0.7492 & 0.8516 & 0.6627 \\
\midrule
6 & 23059 & ESLASSO (supercategory lasso) & \textbf{0.8357} & 0.5940 & 0.5319 \\
6 & 23059 & COMPASSIGN\_PP\_RIDGE (partial pooling ridge) & 0.7706 & \textbf{0.6409} & \textbf{0.5382} \\
\bottomrule
\end{tabular}
\end{table}
...
summary pickles produced by \nolinkurl{external_repos/sally/scripts/sally-dev evaluate}. The exact output directories and
summary pickle paths are recorded under \nolinkurl{external_repos/sally/out/} and are referenced/printed by
\nolinkurl{src/compassign/rt/sally_test.sh}.

\paragraph{Interpretation (current status).}
Across these selected SSIDs, \texttt{COMPASSIGN\_PP\_RIDGE} improves Threat Score (TS) relative to the current lasso
baseline on all three lib208 checks, while the two lib209 checks show mixed behavior (one slight improvement and one
regression). These results are preliminary but are directionally consistent with the offline split evaluations for
lib208: partial pooling with a ridge prior can stabilize RT filtering in hard cell-type supercategories.

\paragraph{Findings so far.}
On these selected cell-type SSIDs, \texttt{COMPASSIGN\_PP\_RIDGE} improves TS by approximately +0.10 (SSID 12307),
+0.06 (SSID 12609), and +0.12 (SSID 12725) versus \texttt{ESLASSO} for lib208.
For lib209, results are mixed: SSID 23059 improves TS slightly (+0.01), while SSID 20159 shows a substantial TS drop
($-0.12$). Overall, the integration appears to be working end-to-end on both libraries, but the performance story is not
uniform for lib209 and requires broader sampling and diagnosis.

\paragraph{Recommended next steps.}
Before making a deployment decision, we recommend:
\begin{itemize}
  \item \textbf{Scale out evaluation.} Run the same baseline-vs-candidate comparison on a larger set of SSIDs from
  supercategories 6 and 8 (and ideally additional cell-type supercategories), reporting both per-SSID and aggregated
  deltas.
  \item \textbf{Scale out lib209 and multi-lib runs.} Repeat the evaluation for lib209 (and for sample sets spanning
  multiple libraries) with the correct per-lib ontology inputs and covariates CSV, and investigate cases where
  \texttt{COMPASSIGN\_PP\_RIDGE} regresses (e.g., SSID 20159) to diagnose alignment/calibration issues.
  \item \textbf{Remove manual inputs.} Implement automatic resolution of \texttt{species} and \texttt{species\_cluster}
  for each SSID (per lib) so that \texttt{sally-dev evaluate} can be run reproducibly without hand-provided IDs.
  \item \textbf{Confirm feature parity at runtime.} Validate that the covariates passed to Sally match CompAssign training
  semantics exactly for all SSIDs, especially around which \texttt{ES\_\*} features are present and how missing standards
  are imputed.
  \item \textbf{Window calibration check.} Confirm that the default interval mapping is appropriate for peak assignment
  (e.g., \texttt{windowMultiplier} behavior and backoff uncertainty choices) and tune/calibrate if needed based on
  aggregate results.
\end{itemize}

\paragraph{Will this generalize to other SSIDs? What remains to make this robust?}
The current integration is designed to generalize to other SSIDs (it scores task-level covariates for arbitrary runs and
computes per-task RT windows), but the prototype still requires manual inputs and alignment work to be production-ready:
\begin{itemize}
  \item \textbf{Ontology alignment (required).} Ensure the \texttt{species} and \texttt{species\_cluster} definitions used
  in CompAssign training are the same ontology that Sally will use at runtime (per lib). This includes stable mappings
  from a sample set to the correct numeric \texttt{species} id and its parent \texttt{species\_cluster}.
  \item \textbf{Automatic species resolution.} Remove the need to pass \texttt{--compassign-rt-species} manually by
  deriving \texttt{species} (and verifying \texttt{species\_cluster}) from Sally inputs or a versioned mapping resource.
  \item \textbf{Feature parity and missing-value policy.} Define a canonical way to build the CompAssign covariates CSV
  for any new SSID, and ensure it matches training conventions exactly: the same set of \texttt{IS*}/\texttt{RS*}/\texttt{ES\_\*}
  columns, the same ordering, and the same missing-value policy (CompAssign training uses 0-imputation for missing
  covariates).
  \item \textbf{Efficient covariates retrieval.} Ensure the runtime interface for task-level covariates is practical for
  production (e.g., a versioned mapping resource/service or a compact per-SSID export). The current CSV-based approach is
  suitable for evaluation and can be made memory-safe by streaming by task ID, but a long-term plan is needed to avoid
  brittle, large-file handling.
  \item \textbf{Window semantics and calibration.} Confirm that the mapping from CompAssign predictive uncertainty to
  Sally's \texttt{windowMultiplier} is calibrated end-to-end (peak assignment is sensitive to windows; overly narrow
  windows reduce recall). If needed, calibrate the multiplier or interval level on held-out sample sets.
  \item \textbf{Multi-lib consistency.} For sample sets spanning multiple libraries, ensure the correct per-lib
  \texttt{species} mapping is available and passed/derived so that lib208/lib209 predictions use the correct ontology and
  artifact bundle.
  \item \textbf{Backoff behavior for unseen groups.} Decide and document how to handle unseen \texttt{(species, comp\_id)}
  groups at runtime (disable backoff vs. back off to hierarchical means), and whether to propagate only observation noise
  or full prior uncertainty into the window for such groups. This choice affects recall/precision trade-offs materially.
  \item \textbf{Artifact packaging and versioning.} Define a reproducible way to ship the \texttt{.npz} artifacts into
  Sally (directory layout, version tags, and validation checks) so that evaluation and production runs use the intended
  model.
\end{itemize}
