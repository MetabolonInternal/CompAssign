...
\begin{align}
  ...
  \epsilon_{scr},
  &
  \epsilon_{scr} &\sim\mathcal{N}\!\bigl(0,\sigma_y^2\bigr),
...
\end{align}
The observation-noise scale $\sigma_y$ is parameterised via
$\log \sigma_y\sim\mathcal{N}(\log 0.1,\,0.5^2)$, which corresponds to a log-normal
prior centred on a modest noise level and spread over an order of magnitude.
...
$\sigma_y^2$ to form a full predictive variance. At prediction time, helper routines
reconstruct the linear predictor for requested (species, compound, run) tuples using
posterior draws of $(\mu_0,\alpha_s,\beta_c,\bm{w}_c,\sigma_y)$ and return both mean
...
\subsection{Model implementation in \texttt{rt\_hierarchical.py}}
...
\texttt{src/compassign/rt\_hierarchical.py} follows this specification closely, using
...
implements $\bm{w}_c^{\top}\bm{x}^{\star}_r$. The final line defines
$y_{scr}\sim\mathcal{N}(\texttt{rt\_mean},\sigma_y^2)$, matching the likelihood in
...
sum-to-zero recentering described earlier. The class/compound hierarchy in
equation~\eqref{eq:rt-compound} is implemented analogously using
\texttt{class\_raw}, \texttt{sigma\_class}, and \texttt{compound\_raw} to construct
\texttt{compound\_eff}.
